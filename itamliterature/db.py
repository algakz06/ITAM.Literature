...
from itamliterature.models.db_models import Book, BookCategory, Base, Voting,\
    VoteCategory, VoteBook, BotUser, VoteResults
...
class DBManager:
    ...
    def get_category_by_index(self, indexes: List[int]) -> List[models.Category]:
        return [models.Category(category)
                for category in self.session.query(BookCategory).filter(
                    BookCategory.id.in_(indexes)
                    ).all()]
        
    def get_categories_with_books(self, categories: List[models.Category]) -> List[models.Category]:
    ...
    def get_current_or_last_voting(self) -> Tuple[str, Voting]:
        if self.session.query(Voting).order_by(desc(Voting.voting_finish)).first() is None:
    ...
    def get_voting_results(self, voting_id: int) -> List[int]:
        result = self.session.query(VoteResults).filter(
            VoteResults.voting_id == voting_id
        ).one()
    ...
    def get_category_votes(self, voting_id: int) -> List[models.Vote]:
        ...
        ]) for vote in self.session.query(VoteCategory).filter(VoteCategory.voting_id == voting_id)]
    ...
    def get_book_votes(self, vote_id: int) -> List[models.Vote]:
        return [models.Vote([
            ...
            vote.second_book_id,
            vote.third_book_id
        ]) for vote in self.session.query(VoteBook).filter(VoteBook.voting_id == vote_id)]
        
    def insert_vote(self, voting_id: int, user_id: int, voting_type: int, vote: models.Vote) -> bool:
        if voting_type == models.Voting.Category.value:
            vote = VoteCategory(voting_id=voting_id,
                              user_id=user_id,
            ...
            vote = VoteBook(voting_id=voting_id,
                            user_id=user_id,
    ...
    def update_voting_results(self, voting_id: int, voting_type: int) -> None:
        ...
        leaders = models.VoteResult([random.choice(leader) for leader in leaders[:3]])
    ...
    def is_admin(self, id: int) -> bool:
        try:
            self.session.query(BotUser).filter(BotUser.telegram_id == id, BotUser.is_admin == True).one()
            return True
        except NoResultFound:
            return False
    #endregion
